...
exports.addAnswer = async (req, res) => {
  try {
    ...
    // answer INSERT 쿼리문
    const query =
      "INSERT INTO answer(qna_id, admin_id, answer_content) VALUES(?, ?, ?)";
    ...
    if (result == 1) {
      ...
      const query =
        "SELECT * FROM answer WHERE qna_id=? AND admin_id=? AND answer_content=?";
      let result = await db(query, [qna_id, admin_id, answer_content]);
...
exports.getAnswer = async (req, res) => {
  try {
    ...
    const query = "SELECT * FROM answer WHERE qna_id=?";
